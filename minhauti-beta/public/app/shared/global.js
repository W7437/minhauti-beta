...
function bedContext(){
  ...
  let patient={};
  try{
    const all=JSON.parse(localStorage.getItem('uti_portatil_v7')||localStorage.getItem('uti_portatil_v3')||'{}')||{};
    patient=all[n]||all[String(n)]||{};
  }catch(e){}
  return {
    type:'minhauti-bed-context',
    bed:n,
    initials:getIni()[n]||'',
    age:patient.age??'',
    weight:patient.weight??'',
    sex:patient.sex??'',
    creatinine:patient.creatinine_now??''
  };
}
...
document.addEventListener('minhauti:auth-ready',function(e){
  ...
  const b=document.createElement('b');b.textContent=u.user_metadata?.username||u.email||'Usuário'; const s=document.createElement('small');s.textContent='Plano: '+plan;
