...
(() => {
  ...
  imageInput.addEventListener('change', () => {
    ...
    imageFileName.textContent = selectedImage ? selectedImage.name : 'Nenhuma imagem selecionada';
    imageGenerateBtn.disabled = !selectedImage || busy;
    imageClearBtn.disabled = !selectedImage || busy;
    if (selectedImage) {
      resetSummaryTable();
      resultEl.textContent = 'Imagem selecionada. Clique em “Ler imagem”.';
      copyBtn.disabled = true;
      hideWarnings();
    }
  });
  ...
  function setBusy(value) {
    ...
    imageGenerateBtn.disabled = value || !selectedImage;
    imageClearBtn.disabled = value || !selectedImage;
  ...
  async function runImageExtraction() {
    ...
    try {
      ...
      setProgress(5, 'Lendo a imagem localmente…');
      const canvas = await renderImageNoRotation(selectedImage);
      log(`Imagem carregada sem rotação: ${canvas.width} × ${canvas.height}px`);
      ...
      console.error(err);
      resultEl.textContent = `ERRO: ${err && err.message ? err.message : String(err)}`;
      warn('O leitor não conseguiu interpretar esta imagem. Abra “Detalhes da leitura” e envie a mensagem ou um print para ajuste do leitor.', true);
      log(`ERRO: ${err && err.stack ? err.stack : err}`);
    } finally {
      setBusy(false);
    }
