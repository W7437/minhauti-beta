...
(() => {
  ...
  imageInput.addEventListener('change', () => {
    ...
    selectedImages = chosen.slice(0, 2);

    if (chosen.length > 2) {
      warn('Selecione no máximo 2 imagens do mesmo paciente.', false);
    } else {
      hideWarnings();
    }
    ...
    if (selectedImages.length === 0) {
      ...
      imageFileName.textContent = selectedImages[0].name;
    } else {
      imageFileName.textContent = selectedImages.map(f => f.name).join(' + ');
    }

    imageGenerateBtn.disabled = selectedImages.length === 0 || busy;
    imageClearBtn.disabled = selectedImages.length === 0 || busy;

    if (selectedImages.length) {
      resetSummaryTable();
      resultEl.textContent = selectedImages.length === 1
        ? 'Imagem selecionada. Clique em “Ler imagem(ns)”.'
        : 'Duas imagens selecionadas. Clique em “Ler imagem(ns)”.';
      copyBtn.disabled = true;
    }
  });

  imageGenerateBtn.addEventListener('click', () => runImageExtraction());
  ...
  function setBusy(value) {
    ...
    imageGenerateBtn.disabled = value || selectedImages.length === 0;
    imageClearBtn.disabled = value || selectedImages.length === 0;
  ...
  function clearImage() {
    ...
    resetSummaryTable();
    resultEl.textContent = 'Selecione um PDF ou uma imagem para começar.';
    debugEl.textContent = '—';
    debugLines = [];
    copyBtn.disabled = true;
    hideWarnings();
  ...
  async function runImageExtraction() {
    ...
    debugEl.textContent = '—';
    resetSummaryTable();
    resultEl.textContent = 'Lendo imagem(ns)…';
    copyBtn.disabled = true;
    setBusy(true);
    ...
    try {
      ...
      for (let i = 0; i < selectedImages.length; i++) {
        ...
        setProgress(10 + (i * 40), `Lendo imagem ${i + 1} de ${selectedImages.length}…`);

        const canvas = await renderImageNoRotation(file);
        log(`Imagem ${i + 1}: ${canvas.width} × ${canvas.height}px`);

        // Primeiro tenta o mesmo detector geométrico usado pelo PDF.
        try {
          const layout = detectLayout(canvas);
          log(`Imagem ${i + 1}: grade reconhecida pelo detector padrão.`);
        } catch (e) {
          log(`Imagem ${i + 1}: detector padrão não localizou a grade (${e.message}).`);
        }

        // OCR global da página para classificar e juntar páginas.
        const rec = await ocrWorker.recognize(canvas, {
          tessedit_pageseg_mode: 6
        });
        const text = rec && rec.data ? (rec.data.text || '') : '';
        mergedTexts.push(text);
      }

      setProgress(85, 'Combinando as páginas…');

      // Junta o texto das duas imagens e tenta aproveitar o parser textual
      // do próprio Auxiliar quando disponível.
      const combinedText = mergedTexts.join('\n\n--- PÁGINA ---\n\n');
      log(`OCR combinado: ${combinedText.length} caracteres.`);

      // Extração tolerante para os principais dados do resumo.
      const values = (regex) => {
        const out = [];
        for (const m of combinedText.matchAll(regex)) {
      ...
      mergedFields.T = values(/(?:TEMPERATURA(?:\(C\))?)[^\n]*?(-?\d{2}(?:[.,]\d)?)/gi);
      mergedFields.FC = values(/(?:FREQU[EÊ]NCIA\s+CARD[IÍ]ACA)[^\n]*?(\d{2,3}(?:[.,]\d)?)/gi);
      mergedFields.FR = values(/(?:FREQU[EÊ]NCIA\s+RESPIRAT[ÓO]RIA)[^\n]*?(\d{1,3}(?:[.,]\d)?)/gi);
      mergedFields.PAS = values(/(?:P\.?\s*A\.?\s*SIST[ÓO]LICA)[^\n]*?(\d{2,3}(?:[.,]\d)?)/gi);
      mergedFields.PAD = values(/(?:P\.?\s*A\.?\s*DIAST[ÓO]LICA)[^\n]*?(\d{2,3}(?:[.,]\d)?)/gi);
      mergedFields.PAM = values(/(?:P\.?\s*A\.?\s*M\.?)[^\n]*?(\d{2,3}(?:[.,]\d)?)/gi);
      mergedFields.SO2 = values(/(?:SATURA[CÇ][AÃ]O\s*O2)[^\n]*?(\d{2,3}(?:[.,]\d)?)/gi);
      mergedFields.HGT = values(/(?:GLICEMIA|HGT)[^\n]*?(\d{2,3}(?:[.,]\d)?)/gi);
      mergedFields.DU = values(/(?:QUANTIDADE\s+)?DIURESE[^\n]*?([+-]?\d+(?:[.,]\d+)?)/gi);

      const bhVals = values(/(?:EVOLU[CÇ][AÃ]O\s*\(?GANHOS.?PERDAS\)?|BALAN[CÇ]O\s+H[IÍ]DRICO)[^\n]*?([+-]?\d+(?:[.,]\d+)?)/gi);

      // Remove chaves vazias.
      for (const k of Object.keys(mergedFields)) {
        if (!mergedFields[k].length) delete mergedFields[k];
      }

      if (!Object.keys(mergedFields).length && !bhVals.length) {
        throw new Error('Nenhum parâmetro conhecido foi reconhecido nas imagens.');
      }

      // Usa o mesmo formato de dados esperado pelo renderizador do Auxiliar.
      const data = {
        fields: mergedFields,
        drains: [],
        evacuations: [],
      ...
      if (bhVals.length) data.balance = bhVals[bhVals.length - 1];

      // Se o renderizador atual aceitar este objeto, usa-o.
      if (typeof renderExtractedData === 'function') {
        renderExtractedData(data);
      }

      // Gera um texto-resumo mínimo e compatível para o campo de evolução.
      const fmtRange = arr => {
      ...
      const parts = [];
      if (mergedFields.T) parts.push(`T ${fmtRange(mergedFields.T)}`);
      if (mergedFields.DU) parts.push(`DU ${mergedFields.DU.reduce((a,b)=>a+b,0)}`);
      if (bhVals.length) parts.push(`BH ${bhVals[bhVals.length-1] >= 0 ? '+' : ''}${bhVals[bhVals.length-1]}`);
      if (mergedFields.FC) parts.push(`FC ${fmtRange(mergedFields.FC)}`);
      if (mergedFields.FR) parts.push(`FR ${fmtRange(mergedFields.FR)}`);
      if (mergedFields.SO2) parts.push(`SO2 ${fmtRange(mergedFields.SO2)}%`);
      if (mergedFields.PAS) parts.push(`PAS ${fmtRange(mergedFields.PAS)}`);
      if (mergedFields.PAD) parts.push(`PAD ${fmtRange(mergedFields.PAD)}`);
      if (mergedFields.PAM) parts.push(`PAM ${fmtRange(mergedFields.PAM)}`);
      if (mergedFields.HGT) parts.push(`HGT ${mergedFields.HGT.join('-')}`);

      resultEl.textContent = parts.join('       ');
      copyBtn.disabled = false;
      setProgress(100, 'Concluído');
      log(`Imagens combinadas: ${selectedImages.length}.`);
    } catch (err) {
      console.error(err);
      log('ERRO:', err && err.stack ? err.stack : err);
      resultEl.textContent = 'Não foi possível concluir a leitura das imagens.';
      warn('O leitor não conseguiu interpretar as imagens. Abra “Detalhes da leitura” e envie a mensagem ou os prints para ajuste do leitor.', true);
    } finally {
      setBusy(false);
    }
  }

  async function renderImageNoRotation(file) {
    const ok = /^(image\/png|image\/jpeg)$/i.test(file.type || '') || /\.(png|jpe?g)$/i.test(file.name || '');
    if (!ok) throw new Error('Formato não suportado. Use PNG, JPG ou JPEG.');
    ...
    try {
      const img = await new Promise((resolve, reject) => {
        ...
        el.onerror = () => reject(new Error('Não foi possível abrir a imagem selecionada.'));
        el.src = url;
      });
