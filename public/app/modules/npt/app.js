...
const MAIN_BEDS_KEY='uti_portatil_v7';
const MAIN_ACTIVE_BED_KEY='minhauti_active_bed_v1';
...
function getMainContext(){
  try{
    const activeRaw=localStorage.getItem(MAIN_ACTIVE_BED_KEY);
    const bed=Math.max(1,parseInt(activeRaw||'1',10)||1);
    const beds=JSON.parse(localStorage.getItem(MAIN_BEDS_KEY)||'{}')||{};
    const data=beds[bed]||beds[String(bed)]||null;
    return {bed,data};
  }catch(e){
    return {bed:null,data:null};
  }
}

function setImportedField(id,value,decimals=null){
  const el=$(id);
  if(!el || value===null || value===undefined || !Number.isFinite(Number(value))) return false;

  const current=String(el.value??'').trim();
  const importedBefore=el.dataset.mainImported==='1';

  // Empty fields or fields still owned by the importer may be refreshed.
  // A manually edited field is preserved.
  if(current!=='' && !importedBefore) return false;

  let text=String(value);
  if(decimals!==null){
    text=Number(value).toFixed(decimals).replace(/\.?0+$/,'');
  }
  el.value=text;
  el.dataset.mainImported='1';
  el.dataset.mainImportedValue=text;
  el.classList.add('from-main');
  return true;
}

function markManualEdit(el){
  if(el.dataset.mainImported==='1'){
    const now=String(el.value??'').trim();
    if(now!==String(el.dataset.mainImportedValue??'')){
      delete el.dataset.mainImported;
      delete el.dataset.mainImportedValue;
      el.classList.remove('from-main');
    }
  }
}

function parseGasometryFallback(raw){
  const text=String(raw||'');
  const pick=(patterns)=>{
    for(const re of patterns){
      const m=text.match(re);
      if(m){
        const v=finiteValue(m[1]);
        if(v!==null) return v;
      }
    }
    return null;
  ...
  return {
    hco3: pick([/\bHCO3[-⁻]?\s*[:=]?\s*(-?\d+(?:[.,]\d+)?)/i, /\bBIC(?:ARBONATO)?\s*[:=]?\s*(-?\d+(?:[.,]\d+)?)/i]),
    ph: pick([/\bpH\s*[:=]?\s*(\d+(?:[.,]\d+)?)/i])
  };
}

function importFromMain(){
  const status=$('mainImportStatus');
  const {bed,data}=getMainContext();

  if(!data){
    if(status) status.textContent='Nenhum dado salvo encontrado para o leito ativo.';
    return;
  }

  let applied=0;

  for(const [target,source] of Object.entries(mainFieldMap)){
    let value=finiteValue(data[source]);
    if(value===null && (target==='bic' || target==='ph')){
      const gas=parseGasometryFallback(data.gasometry_text);
      value=target==='bic'?gas.hco3:gas.ph;
    }
    if(value!==null && setImportedField(target,value)) applied++;
  }

  // Diurese: NPT asks specifically for a 24 h volume.
  const du=finiteValue(data.urine_volume);
  const hours=finiteValue(data.urine_hours);
  if(du!==null && hours===24){
    if(setImportedField('diurese',du)) applied++;
  }

  // BH is categorical in NPT. It is safe to use only the sign of a documented balance.
  const bh=finiteValue(data.fluid_balance);
  const bhEl=$('bh');
  if(bhEl && bh!==null){
    const importedBefore=bhEl.dataset.mainImported==='1';
    if(importedBefore || !bhEl.dataset.userChanged){
      bhEl.value=bh>0?'positivo':(bh<0?'negativo':'neutro');
      bhEl.dataset.mainImported='1';
      bhEl.dataset.mainImportedValue=bhEl.value;
      bhEl.classList.add('from-main');
      applied++;
    }
  }

  // If the main explicitly says the patient is on HD, reflect it.
  const renalEl=$('renal');
  if(renalEl && data.hemodialysis===true){
    const importedBefore=renalEl.dataset.mainImported==='1';
    if(importedBefore || !renalEl.dataset.userChanged){
      renalEl.value='ihd';
      renalEl.dataset.mainImported='1';
      renalEl.dataset.mainImportedValue='ihd';
      renalEl.classList.add('from-main');
      applied++;
    }
  }
  ...
  if(status){
    status.textContent = applied
      ? `Dados importados do leito ${bed}. Valores continuam editáveis.`
      : `Leito ${bed} encontrado. Nenhum campo novo foi sobrescrito; alterações manuais foram preservadas.`;
  }
}

function setupMainIntegration(){
  const ids=['peso','na','k','mg','p','cr','bic','ph','diurese'];
  ids.forEach(id=>{
    const el=$(id);
    if(!el) return;
    el.addEventListener('input',()=>markManualEdit(el));
  });
  ...
  ['bh','renal'].forEach(id=>{
    ...
    el.addEventListener('change',()=>{
      if(el.dataset.mainImported==='1' && el.value!==el.dataset.mainImportedValue){
        delete el.dataset.mainImported;
        delete el.dataset.mainImportedValue;
        el.classList.remove('from-main');
      }
      el.dataset.userChanged='1';
    });
  });

  $('importMainData')?.addEventListener('click',importFromMain);
  importFromMain();
}
...
$('limpar').addEventListener('click',()=>{
 document.querySelectorAll('input').forEach(x=>{x.value='';delete x.dataset.mainImported;delete x.dataset.mainImportedValue;x.classList.remove('from-main')});
